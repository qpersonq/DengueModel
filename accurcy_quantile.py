...
with open(str(ExcelFileName)+".csv","w") as qwr:
	qwr.write("QT,enva,envd,posa,posd,enla,enld\n")
	qparameter=[0.0,0.25,0.5,0.75,1];
	for qp in qparameter:
		qwr.write(str(qp))
		qwr.write(",")
		qwr.write(str(np.quantile(enva,qp)))
		qwr.write(",")
		qwr.write(str(np.quantile(envd,qp)))
		qwr.write(",")
		qwr.write(str(np.quantile(posa,qp)))
		qwr.write(",")
		qwr.write(str(np.quantile(posd,qp)))
		qwr.write(",")
		qwr.write(str(np.quantile(enla,qp)))
		qwr.write(",")
		qwr.write(str(np.quantile(enld,qp)))
		qwr.write("\n")
