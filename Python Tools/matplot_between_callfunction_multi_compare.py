...
def calculateLowerAndUpperBound(ExcelContainerCollection ,Mode,AxisXLabelMaxMin=None,ForSaveFid=0):
	'''AxisXLabelMaxMinMin Max'''
	plt.figure(figsize=(6,5))
	ColorSeriesForChart=['blue','red','lime','g','c','m']
	CurrentExcelFileNumber=0
	for ExcelContainer in ExcelContainerCollection:
		...
		if(Mode=='l'):
			...
			yend=52
			yinterval=2
			ubcolor=ColorSeriesForChart[CurrentExcelFileNumber%len(ColorSeriesForChart)]
			#lbcolor="orange"
			'''lag'''
...
ExcelContainer=[]
while True:
	ExcelFileName=input("Quartile Excel (-1 end)>>");
	if ExcelFileName=="-1":
	 break;
...
calculateLowerAndUpperBound(ExcelContainer,"l",CXMinMax,"CONLag");
calculateLowerAndUpperBound(ExcelContainer,"d",CXMinMax,"CONDist");